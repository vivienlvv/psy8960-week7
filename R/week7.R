...
# Visualization- Still need to follow line instruction, also need to change size of graphs (too big)
...
(week7_tbl %>% ggplot(aes(x = timeStart, y = q1)) + # Figure 1 
  ...
  theme(axis.title = element_text(face = "bold"), aspect.ratio = 6/10)) %>%
  ggsave(filename = "../figs/fig1.png")

(week7_tbl %>% ggplot(aes(x = q1, y = q2, color = gender)) + # Figure 2 
  geom_point(position = "jitter") + 
  labs(color = "Participant Gender") + 
  theme(axis.title =  element_text(face = "bold"), legend.text = element_text(face = "bold"), legend.title = element_text(face = "bold"), aspect.ratio = 7/10)) %>% 
...
(week7_tbl %>% ggplot(aes(x = q5, y = q7, color = condition, fill = condition)) + # Figure 5 
  ...
  theme(legend.position ="bottom",
        ...
        axis.title = element_text(face = "bold"), legend.title = element_text(face = "bold"), legend.text = element_text(face = "bold"), aspect.ratio = 3/7)) %>% 
